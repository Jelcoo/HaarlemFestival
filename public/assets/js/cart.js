...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function createDanceCardHTML(event, startTime, endTime) {
    ...
    <div class="d-flex">
        ...
        <div class="counter">
            <button type="button" class="decrease-btn" data-type="dance" data-id="${
              event.event_id
            }">
                <i class="fa-solid fa-minus"></i>
            </button>
            <span>${event.quantity}</span>
            <button type="button" class="increase-btn" data-type="dance" data-id="${
              event.event_id
            }">
                <i class="fa-solid fa-plus"></i>
            </button>
        </div>
        <button type="button" class="remove-btn" data-type="dance" data-id="${
          event.event_id
        }">
  ...
  function createYummyCardHTML(event, startTime, endTime) {
    ...
    <div>
        ...
        <div>
            <p>Duration: ${startTime}-${endTime}</p>
            <p>Reservation cost: €${
              event.reservationcost ? event.reservationcost : "N/A"
            }</p>
    ...
    <div class="d-flex">
        <div class="d-flex flex-column align-items-stretch p-0 gap-2 justify-content-between" style="flex-grow: 0.5">
            <div class="d-flex justify-content-between p-0">
                ...
                <div class="counter">
                    <button type="button" class="decrease-btn" data-type="yummy" data-id="${
                      event.event_id
                    }" data-category="adult">
                        <i class="fa-solid fa-minus"></i>
                    </button>
                    <span>${event.adult_quantity}</span>
                    <button type="button" class="increase-btn" data-type="yummy" data-id="${
                      event.event_id
                    }" data-category="adult">
                        <i class="fa-solid fa-plus"></i>
                    </button>
        ...
        <button type="button" class="remove-btn align-self-end" data-type="yummy" data-id="${
          event.event_id
        }">
            <i class="fa-solid fa-trash"></i>
        </button>
...
`;
  ...
  function updateQuantity(type, id, change, category = null) {
    ...
    const item = items.find((item) => {
      if (Array.isArray(id)) {
        return JSON.stringify(item.event_id) === JSON.stringify(id);
      } else {
